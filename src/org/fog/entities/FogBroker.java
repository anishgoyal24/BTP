...
public class FogBroker extends PowerDatacenterBroker{
    ...
    public static int count=0;//During initialization, count which particle is currently used to allocate virtual machines to the job
    public static int count2=0;//When updating particles, count which particle is currently assigned to the virtual machine for the job
                ...
                switch (Parameters.getSchedulingAlgorithm()) {
                    case MOM:
                        if(WorkflowEngine.updateFlag==0&&WorkflowEngine.startlastSchedule==0) {
                            processCloudletUpdateForMomentumSAInit(ev);
                        }else if(WorkflowEngine.startlastSchedule==0){
                            processCloudletUpdateForMomentumSAUpdate(ev);
                        }else {
                            processCloudletUpdateForMomentumSAGbest(ev);
                        }
                        break;
                    case PSO:
                        if(WorkflowEngine.updateFlag==0&&WorkflowEngine.startlastSchedule==0) {
                    ...
                    case GWO:
                        if(WorkflowEngine.updateFlag==0&&WorkflowEngine.startlastSchedule==0) {
                            ...
                            processCloudletUpdateForGWOGbest(ev);
                        }
                        break;
                    case GOA:
                        if(WorkflowEngine.updateFlag==0&&WorkflowEngine.startlastSchedule==0) {
                    ...
                    case MS:
                        if(WorkflowEngine.updateFlag==0&&WorkflowEngine.startlastSchedule==0) {
                            processCloudletUpdateForMSInit(ev);
                        }else if(WorkflowEngine.startlastSchedule==0){
                            processCloudletUpdateForMSUpdate(ev);
                        }else {
                            processCloudletUpdateForMSGbest(ev);
                        }
                        break;
				case MINMIN:
				case MAXMIN:
...
//            System.out.println(cloudlet.getCloudletId()+".submit delay : "+delay);
    ...
    protected void processCloudletUpdateForMomentumSAInit(SimEvent ev) {
        List<Cloudlet> cloudletList=getCloudletList();
        List<CondorVM> vmList=getVmsCreatedList();
        if(MomentumSA.initFlag==0) {
            startTime = System.currentTimeMillis();
            WorkflowEngine engine = (WorkflowEngine)CloudSim.getEntity(workflowEngineId);
            MomentumSA.init(engine.jobList.size(),getVmList().size());
        }
        List<Cloudlet> scheduledList =new ArrayList<Cloudlet>();
        List<int[]> schedules=MomentumSA.schedules;
        for(int i=0;i<cloudletList.size();i++) {
            int cloudletId=cloudletList.get(i).getCloudletId();
            int vmId=schedules.get(count)[cloudletId];
            cloudletList.get(i).setVmId(vmId);
    ...
    protected void processCloudletUpdateForMomentumSAUpdate(SimEvent ev) {
        List<Cloudlet> cloudletList=getCloudletList();
        List<CondorVM> vmList=getVmsCreatedList();
        if(WorkflowEngine.updateFlag2==1&&cloudletList.size()!=0) {
            MomentumSA.updateBodies();
        }
        List<Cloudlet> scheduledList =new ArrayList<Cloudlet>();
        List<int[]> newSchedules=MomentumSA.schedules;
        for(int i=0;i<cloudletList.size();i++) {
            int cloudletId=cloudletList.get(i).getCloudletId();
            int vmId=newSchedules.get(count2)[cloudletId];
            cloudletList.get(i).setVmId(vmId);
    ...
    protected void processCloudletUpdateForMomentumSAGbest(SimEvent ev) {
        List<Cloudlet> cloudletList=getCloudletList();
        List<CondorVM> vmList=getVmsCreatedList();
        List<Cloudlet> scheduledList =new ArrayList<Cloudlet>();
        for(int i=0;i<cloudletList.size();i++) {
            int cloudletId=cloudletList.get(i).getCloudletId();
            int vmId=MomentumSA.gbest_schedule[cloudletId];
            cloudletList.get(i).setVmId(vmId);
            //setVmState(vmId);
            scheduledList.add(cloudletList.get(i));
        }
    ...
    protected void processCloudletUpdateForGWOInit(SimEvent ev) {
        ...
        if(GWO.initFlag==0) {
            startTime = System.currentTimeMillis();
            WorkflowEngine engine = (WorkflowEngine)CloudSim.getEntity(workflowEngineId);
            GWO.init(engine.jobList.size(),getVmList().size());
        }
        List<Cloudlet> scheduledList =new ArrayList<Cloudlet>();
        List<int[]> schedules=GWO.schedules;
        for(int i=0;i<cloudletList.size();i++) {
            int cloudletId=cloudletList.get(i).getCloudletId();
            int vmId=schedules.get(count)[cloudletId];
            cloudletList.get(i).setVmId(vmId);
    ...
    protected void processCloudletUpdateForGWOUpdate(SimEvent ev) {
        ...
        List<CondorVM> vmList=getVmsCreatedList();
        if(WorkflowEngine.updateFlag2==1&&cloudletList.size()!=0) {
            GWO.updateWolves(); // CHECK ITERATION NUMBER TO BE PASSED
        }
        List<Cloudlet> scheduledList =new ArrayList<Cloudlet>();
        List<int[]> newSchedules=GWO.schedules;
        for(int i=0;i<cloudletList.size();i++) {
            int cloudletId=cloudletList.get(i).getCloudletId();
            int vmId=newSchedules.get(count2)[cloudletId];
            cloudletList.get(i).setVmId(vmId);
    ...
    protected void processCloudletUpdateForGWOGbest(SimEvent ev) {
        List<Cloudlet> cloudletList=getCloudletList();
        List<CondorVM> vmList=getVmsCreatedList();
        List<Cloudlet> scheduledList =new ArrayList<Cloudlet>();
        for(int i=0;i<cloudletList.size();i++) {
            int cloudletId=cloudletList.get(i).getCloudletId();
            int vmId=GWO.gbest_schedule[cloudletId];
            cloudletList.get(i).setVmId(vmId);
            //setVmState(vmId);
            scheduledList.add(cloudletList.get(i));
        }
    ...
    protected void processCloudletUpdateForGOAInit(SimEvent ev) {
        List<Cloudlet> cloudletList=getCloudletList();
        List<CondorVM> vmList=getVmsCreatedList();
        if(GOA.initFlag==0) {
            startTime = System.currentTimeMillis();
    ...
    protected void processCloudletUpdateForGOAGbest(SimEvent ev) {
        List<Cloudlet> cloudletList=getCloudletList();
        List<CondorVM> vmList=getVmsCreatedList();
        List<Cloudlet> scheduledList =new ArrayList<Cloudlet>();
        for(int i=0;i<cloudletList.size();i++) {
            int cloudletId=cloudletList.get(i).getCloudletId();
            int vmId=GOA.targetPosition[cloudletId];
            cloudletList.get(i).setVmId(vmId);
            //setVmState(vmId);
            scheduledList.add(cloudletList.get(i));
        }
    ...
    protected void processCloudletUpdateForMSInit(SimEvent ev) {
        List<Cloudlet> cloudletList=getCloudletList();
        List<CondorVM> vmList=getVmsCreatedList();
        if(MSAlgorithm.initFlag==0) {
            startTime = System.currentTimeMillis();
            WorkflowEngine engine = (WorkflowEngine)CloudSim.getEntity(workflowEngineId);
            MSAlgorithm.initPopsRandomly(engine.jobList.size(),getVmList().size());
        }
        List<Cloudlet> scheduledList =new ArrayList<Cloudlet>();
        ArrayList<Moth> schedules=MSAlgorithm.schedules;
        for(int i=0;i<cloudletList.size();i++) {
            int cloudletId=cloudletList.get(i).getCloudletId();
            int vmId=schedules.get(count).getPosition()[cloudletId];
            cloudletList.get(i).setVmId(vmId);
    ...
    protected void processCloudletUpdateForMSUpdate(SimEvent ev) {
        //System.out.println("update");
        List<Cloudlet> cloudletList=getCloudletList();
        List<CondorVM> vmList=getVmsCreatedList();
        if(WorkflowEngine.updateFlag2==1&&cloudletList.size()!=0) {
            MSAlgorithm.algo(); // CHECK ITERATION NUMBER TO BE PASSED
        }
        List<Cloudlet> scheduledList =new ArrayList<Cloudlet>();
        List<Moth> newSchedules=MSAlgorithm.schedules;
        for(int i=0;i<cloudletList.size();i++) {
            int cloudletId=cloudletList.get(i).getCloudletId();
            int vmId=newSchedules.get(count2).getPosition()[cloudletId];
            cloudletList.get(i).setVmId(vmId);
    ...
    protected void processCloudletUpdateForMSGbest(SimEvent ev) {
        List<Cloudlet> cloudletList=getCloudletList();
        List<CondorVM> vmList=getVmsCreatedList();
        List<Cloudlet> scheduledList =new ArrayList<Cloudlet>();
        for(int i=0;i<cloudletList.size();i++) {
